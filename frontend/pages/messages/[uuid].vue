    ...
    <MainLayout :page-title="pageTitle">
        <div v-if="!pending">
                ...
                <template #header>
                    <div class="flex justify-between">
                        ...
                        <UDropdown :items="messageActions" mode="click">
                            <!-- <UButton color="white" label="Actions" class="lg:hidden" trailing-icon="i-heroicons-chevron-down-20-solid" /> -->
                            <UButton color="white" label="Actions" trailing-icon="i-heroicons-chevron-down-20-solid" />
                        </UDropdown>
                        <!-- <div class="hidden lg:flex lg:justify-between lg:space-x-2">
                            <UButton color="gray" trailing-icon="i-heroicons-eye" :loading="headers_pending" :disabled="pending" label="View Headers" @click="showHeaders = true"/>
                            <UButton color="gray" trailing-icon="i-heroicons-eye" :loading="headers_pending" :disabled="pending" label="View Contents" @click="showContents = true"/>
                            <UButton color="red" trailing-icon="i-heroicons-hand-thumb-down" :disabled="pending" label="Mark as spam"/>
                            <UButton color="green" trailing-icon="i-heroicons-hand-thumb-up" :disabled="pending" label="Mark as safe"/>
                            <UButton color="blue" trailing-icon="i-heroicons-envelope" :disabled="pending" label="Release"/>
                        </div> -->
                    </div>
                </template>
                <div class="mt-6">
            ...
            <UCard>
                ...
                <UTable :loading="logs_pending" :columns="logsColumns" :rows="logs" />
            </UCard>
            <UModal v-model="showHeaders" :ui="{ width: 'w-full sm:max-w-2xl md:max-w-4xl lg:max-w-6xl 2xl:max-w-screen-2xl' }">
                <UCard>
                    ...
                    <UTable :loading="headers_pending" :columns="headerColumns" :rows="headers"/>
                </UCard>
            </UModal>
            ...
            <UModal v-model="showContents" :ui="{ width: 'w-full sm:max-w-2xl md:max-w-4xl lg:max-w-6xl 2xl:max-w-screen-2xl' }">
                <UCard>
                    ...
                    <!-- <UTable :loading="contents_pending" :columns="headerColumns" :rows="contents"/> -->
                </UCard>
            </UModal>
            ...
            <UModal v-model="showSpamAnalysis" :ui="{ width: 'w-full sm:max-w-2xl md:max-w-4xl lg:max-w-6xl 2xl:max-w-screen-2xl' }">
                <UCard>
                    ...
                    <UTable :loading="spamreport_pending" :columns="spamReportColumns" :rows="spamreport"/>
                </UCard>
            </UModal>
        </div>
        <UCard v-else="">
...
<script setup>
    ...
    const { pending, data } = (await useApi(`/messages/${uuid}`))

    const { pending: headers_pending, data: headers } = (await useApi(`/messages/${uuid}/headers`))
    const { pending: spamreport_pending, data: spamreport } = (await useApi(`/messages/${uuid}/reports/spam`))
    const { pending: logs_pending, data: logs } = (await useApi(`/messages/${uuid}/logs`))
    const { pending: contents_pending, data: contents } = (await useApi(`/messages/${uuid}/view`))
