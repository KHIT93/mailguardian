...
class DataLogModelRegistry(object):
    """
    A registry that keeps track of the models that use datalog to track changes.
    """
    ...
    def register(self, model: ModelBase = None, include_fields: Optional[List[str]] = None,
        ...
        """
        Register a model with datalog. DataLog will then track mutations on this model's instances.
        ...
        def registrar(cls):
            ...
            # syntactic sugar for:
            # MyClass = datalog.register(MyClass)
            return cls
    ...
    def contains(self, model: ModelBase) -> bool:
        """
        Check if a model is registered with datalog.
    ...
    def unregister(self, model: ModelBase) -> None:
        """
        Unregister a model with datalog. This will not affect the database.
