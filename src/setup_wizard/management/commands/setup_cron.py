...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        job_hour = cron.new(command='{0} {1} runjobs hourly > /dev/null 2>&1'.format(which('python'), os.path.join(settings.BASE_DIR, 'manage.py')))
        job_hour.every().hours()
        job_day = cron.new(command='{0} {1} runjobs daily > /dev/null 2>&1'.format(which('python'), os.path.join(settings.BASE_DIR, 'manage.py')))
        job_day.every().day()
        job_week = cron.new(command='{0} {1} runjobs weekly > /dev/null 2>&1'.format(which('python'), os.path.join(settings.BASE_DIR, 'manage.py')))
