...
export default {
    ...
    methods: {
        get() {
            ...
            (new Form(this.activeFilters)).post('/api/reports/top-recipient-domains-by-quantity/').then(data => {
                this.data = data;
                this.chart = {
                    labels: this.data.map(item => item.to_domain),
                    datasets: [
                        {
                            backgroundColor: [
                                '#F7b7b7',
                            ...
                            data: this.data.map(item => item.id__count)
                        }
                    ]
                }
                this.toggleLoading();
