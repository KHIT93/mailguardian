...
class AuditLog(SQLModel):
    ...
    res_id: int | None = Field(description='The ID of the record in the database model, on which the action was performed')

    action: AuditAction

    actor_id: int | None = Field(foreign_key="users.id", description='The ID of the user who performed the action')
    ...
    changes: str | None = Field(description='JSON object with the values that changed')

    message: str | None = Field(description='A message connected to the auditted action')
