...
class SQLModelStorage(BaseStorage):
    ...
    def enqueue(self, data: bytes, priority: float | None = 0) -> int:
    ...
    def enqueued_items(self, limit: int | None = None) -> list[bytes]:
    ...
    def scheduled_items(self, limit: int | None = None) -> list[bytes]:
        query = self.schedule()
    ...
    def result_items(self) -> dict:
        with Session(engine) as db:
            return {r.key: r.value for r in db.exec(self.kv()).all()}
