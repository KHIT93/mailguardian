...
from mailguardian.config.app import FRONTEND_DIR, settings
...
def rebuild(debug: Annotated[bool, typer.Option('--debug', help='Used to render detailed command output')] = False):
    if not settings.NUXT_BIN.is_file():
        ...
        typer.Exit(1)
    if debug:
        invoke.run(f'cd {FRONTEND_DIR} && {settings.NUXT_BIN} build', hide=False)
    else:
        with Progress(SpinnerColumn(), TextColumn('[progress.description]{task.description}'), transient=True) as progress:
            progress.add_task(description='Rebuilding client application...', total=None)
            invoke.run(f'cd {FRONTEND_DIR} && {settings.NUXT_BIN} build', hide=True)
