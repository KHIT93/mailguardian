...
from mailguardian.app.auth.utils import (
    hash_password,
...
from mailguardian.app.schemas.user import (
    ChangePassword,
    PersonalDetails,
    TokenData,
    UserRole,
...
async def terminate_sessions(db: Annotated[Session, Depends(get_database_session)], request: Request, authenticated_user: Annotated[User, Depends(get_current_user)]) -> None:
    db.exec(delete(UserSession).where(UserSession.user_id == authenticated_user.id))
