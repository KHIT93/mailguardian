...
async def store(db: Annotated[Session, Depends(get_database_session)], request: Request, authenticated_user: Annotated[User, Depends(get_current_user)], data: ListEntrySchema) -> ListEntry:
    ...
    db.add(res)
    db.commit()
    audit_interaction(request=request, action=AuditAction.CREATE, model=ListEntry.__name__, res_id=res.id, actor_id=authenticated_user.id, message=f'Created Blocked Sender ({res.id})')
    db.refresh(res)
...
async def update(db: Annotated[Session, Depends(get_database_session)], request: Request, authenticated_user: Annotated[User, Depends(get_current_user)], id: uuid.UUID, data: ListEntrySchema) -> ListEntry:
    ...
    db.add(res)
    db.commit()
    audit_interaction(request=request, action=AuditAction.UPDATE, model=ListEntry.__name__, res_id=res.id, actor_id=authenticated_user.id, message=f'Updated Blocked Sender ({res.id})', old=res.model_dump(), new=data.model_dump())
    db.refresh(res)
...
async def partial_update(db: Annotated[Session, Depends(get_database_session)], request: Request, authenticated_user: Annotated[User, Depends(get_current_user)], id: uuid.UUID, data: ListEntrySchema) -> ListEntry:
    ...
    db.add(res)
    db.commit()
    audit_interaction(request=request, action=AuditAction.UPDATE, model=ListEntry.__name__, res_id=res.id, actor_id=authenticated_user.id, message=f'Updated Blocked Sender ({res.id})', old=res.model_dump(), new=data)
    db.refresh(res)
...
async def destroy(db: Annotated[Session, Depends(get_database_session)], request: Request, authenticated_user: Annotated[User, Depends(get_current_user)], id: uuid.UUID) -> None:
    ...
    db.delete(res)
    db.commit()
    audit_interaction(request=request, action=AuditAction.DELETE, model=ListEntry.__name__, res_id=res_id, actor_id=authenticated_user.id, message=f'Deleted Blocked Sender ({res_id})', old=data, new={})
