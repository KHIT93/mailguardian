...
async def index(db: Annotated[Session, Depends(get_database_session)], authenticated_user: Annotated[User, Depends(get_current_user)], q: Annotated[str, Query()] = '', page: Annotated[int, Query(ge=1)] = 1, per_page: Annotated[int, Query(ge=0)] = 20) -> PaginatedResponse[ListEntry]:  # noqa: ARG001
    ...
    if authenticated_user.role == UserRole.DOMAIN_ADMINISTRATOR:
        query = select(ListEntry).where(
            ListEntry.listing_type == ListingType.ALLOWED and
        ...
        query = select(ListEntry).where((ListEntry.from_address == authenticated_user.email or ListEntry.to_address == authenticated_user.email) and ListEntry.listing_type == ListingType.ALLOWED)
    if q:
    ...
    if authenticated_user.role == UserRole.USER:
        if authenticated_user.email not in [data.from_address, data.to_address]:
            raise HTTPException(
                ...
                detail='You cannot create a allowlist entry, which does not invovle your own email address'
            )
