...
from mailguardian.app.auth.totp import generate_secret, generate_totp_url, verify_totp
...
from mailguardian.app.models.user import TimeBasedCode, User
...
def get_totp_devices(db: Annotated[Session, Depends(get_database_session)], request: Request, authenticated_user: Annotated[User, Depends(get_current_user)]) -> list[TimeBasedCode]:
    ...
    audit_interaction(request=request, action=AuditAction.READ, model=TimeBasedCode.__name__, res_id=authenticated_user.id, actor_id=authenticated_user.id, message='Fetched TOTP Codes')
...
def enable_totp(request: Request, authenticated_user: Annotated[User, Depends(get_current_user)]) -> TotpSetup:
