<template>
    <MainLayout>
        <div class="bg-white p-4 shadow border rounded-md space-y-8 sm:space-y-5 divide-y divide-gray-200 relative pb-8">
                ...
                <tbody class="bg-white">
                    ...
                    <tr v-for="domain in filteredEntries" :key="domain.id" v-else class="cursor-pointer border-b transition duration-300 hover:shadow-lg hover:font-bold">
                        ...
                        <td class="px-6 whitespace-nowrap text-right text-sm font-medium">
                            <router-link :to="`/domains/${domain.id}/edit`" class="text-blue-600 hover:text-blue-900">
                                <PencilIcon class="w-4 h-4"/>
                            </router-link>
                        </td>
                        <td class="px-6 whitespace-nowrap text-right text-sm font-medium">
                            <router-link :to="`/domains/${domain.id}/delete`" class="text-red-600 hover:text-red-900">
            ...
            <router-link to="/domains/create" class="absolute right-4 -bottom-5 inline-flex items-center p-2 border border-transparent rounded-full shadow hover:shadow-lg transition-all duration-300 text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
...
export default {
    ...
    setup(props) {
        ...
        let fetchEntries = (async () => {
            loading.value = true
            let res = (await axios.get(`/api/domains/?page_size=20`)).data.results
            loading.value = false
            return res
        })
