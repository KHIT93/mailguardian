<template>
    <MainLayout>
        ...
        <div class="shadow overflow-hidden sm:rounded-md">
            <UCard>
                <template #header>
                    <h1 class="text-xl">Delete user ({{ id }})?</h1>
                </template>
                <p>
                    Are you absoutely certain that you wish to delete this user?<br/>
...
useHead({
    title: 'MailGuardian - Delete User'
})
...
async function submit() {
    await form.delete(`/api/users/${id}/`)
    navigateTo('/users')
}
</script>